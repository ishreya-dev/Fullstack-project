...
class AuditLogRepository:
    ...
    async def _get_files_matching_pattern(
        ...
        logger.info(
            f"Reading audit logs from files matching pattern: {file_path_pattern}"
        )
    ...
    async def list_audit_logs(
        ...
        total = 0
        keep = skip + limit
        ring: deque[dict[str, str]] = deque(maxlen=keep)  # keep the last (skip + limit) lines
    ...
    async def return_all_audit_logs(
        self, action: Optional[AuditActionType] = None, tenant_id: str | None = None
    ) -> List[AuditLogDto]:
        files = await self._get_files_matching_pattern(tenant_id=tenant_id)
