...
async def get_dashboard_metrics(
    ...
    pipeline = [
        {"$match": match_stage} if match_stage else {"$match": {}},
        {
            "$group": {
                "_id": {
                    "$dateToString": {"format": group_format, "date": "$created_at"}
                },
                "count": {"$sum": 1},
            }
        },
        {"$sort": {"_id": 1}},
        {"$project": {"time_or_date": "$_id", "count": "$count", "_id": 0}},
    ]

    timeseries = await user_service.aggregate(pipeline)
    total_users = await user_service.total_count()
    joined_users = (
        await user_service.total_count(match_stage) if match_stage else total_users
    )
