...
from api.domain.dtos.billing_dto import (
    CreatePlanDto,
    PlanDto,
    PlanListDto,
    UpdatePlanDto,
...
router = APIRouter(
    prefix="/billing",
    dependencies=[Depends(check_feature_access(feature_name=Feature.STRIPE))],
)
...
@router.get(
    "/plans/{plan_id:path}", summary="Retrieve a plan details", response_model=PlanDto
)
async def get_plan(
    plan_id: str,
    current_user_optional: CurrentUserOptional,
...
@router.patch(
    "/plans/{plan_id:path}",
    summary="Update a existing plan",
...
async def update_plan(
    ...
    await billing_service.update_plan(
        plan_id=plan_id, update_plan=updated_plan, scope=scope
    )
