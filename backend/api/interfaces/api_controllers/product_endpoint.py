    ...
    dependencies=[
        ...
        Depends(
            check_permissions_for_current_role(
                required_permissions=[Permission.MANAGE_PRODUCTS_AND_PRICING]
            )
        ),
    ],
)
...
async def list_products(
    ...
    active: bool = Query(
        default=True, description="Filter active products, defaults to True"
    ),
):
...
@router.get(
    "/{product_id:path}", summary="Get Product by ID", response_model=ProductDto
)
...
async def update_product(
    ...
    await product_service.update_product(
        product_id=product_id, product_dto=product_dto, scope=scope
    )
