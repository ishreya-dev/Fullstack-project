...
@router.get(
    "/",
    response_model=List[AvailableStorageProviderDto],
...
async def configure_storage(
    ...
    try:
        ...
        await setting_service.configure_storage(
            setting=configuration, tenant_id=current_user.tenant_id
        )
...
async def reset_storage(
    ...
    try:
        ...
        await setting_service.reset_storage(
            storage_id=storage_id,
            updated_by_user_id=current_user.id,
            tenant_id=PydanticObjectId(current_user.tenant_id),
        )
        return status.HTTP_201_CREATED
