...
async def get_registered_passkeys(
    ...
    await audit_log_service.create_audit_log(
        audit_log=AuditLogDto(
            ...
            action="read",
            changes={
                "Info": f"Retrieved {len(res)} passkeys for user {current_user.email}."
            },
    ...
    try:
        ...
        await audit_log_service.create_audit_log(
            audit_log=AuditLogDto(
                ...
                action="delete",
                changes={
                    "Info": f"Deleted passkey {credential_id} for user {current_user.email}."
                },
                user_id=current_user.id,
                tenant_id=current_user.tenant_id,
            )
        ...
        await audit_log_service.create_audit_log(
            audit_log=AuditLogDto(
                ...
                action="error",
                changes={
                    "Info": f"Failed to delete passkey {credential_id} for user {current_user.email}."
                },
                user_id=current_user.id,
                tenant_id=current_user.tenant_id,
            )
