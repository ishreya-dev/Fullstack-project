...
from api.domain.dtos.audit_logs_dto import (
    AuditActionType,
    AuditLogListDto,
    DownloadAuditRequestDto,
)
    ...
    dependencies=[
        Depends(
            check_permissions_for_current_role(
                required_permissions=[Permission.AUDIT_LOGS_VIEW_ONLY]
            )
        )
    ],
)
...
async def list_audit_logs(
    ...
    return await log_service.read_audit_logs(
...
@router.post(
    ...
    response_model=DownloadResponseDto,
)
async def download_audit_logs(
    ...
    _bool: bool = Depends(
        check_permissions_for_current_role(
            required_permissions=[Permission.AUDIT_LOGS_DOWNLOAD]
        )
    ),
):
    if download_request.total_records <= 0:
        raise InvalidOperationException(
            "Audit log download request must have at least one record."
        )
    ...
    return DownloadResponseDto(
        message="Download request has been received. You will receive an email once the report is ready."
    )
