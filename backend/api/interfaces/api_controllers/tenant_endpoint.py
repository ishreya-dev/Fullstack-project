...
async def delete_tenant(
    ...
    try:
        ...
        payload = WorkerPayloadDto(
            label="post-tenant-deletion", data=tenant_doc, tenant_id=id
        )
...
@router.get(
    "/{tenant_id}/features",
    response_model=list[FeatureDto],
    status_code=status.HTTP_200_OK,
)
...
async def activate_trial_plan(
    tenant_id: str,
    tenant_service: TenantService = Depends(get_tenant_service),
    subcription_plan_service: SubscriptionPlanService = Depends(
        get_subscription_plan_service
    ),
    ...
    if len(all_plans.plans) == 0:
        logger.error(
            "No plans are configured in Stripe. Please create a plan before activating a trial."
        )
        raise InvalidOperationException(
            "No plans are configured. Please create a plan before activating a trial."
        )

    trial = list(
        filter(
            lambda p: p.trial_period_days is not None and p.trial_period_days > 0,
            all_plans.plans,
        )
    )
    ...
    if not trial:
        logger.error(
            "No trial plan is configured. Please create a plan with a trial period before activating a trial or update an existing plan."
        )
        raise InvalidOperationException(
            "No trial plan is configured. Please create a plan with a trial period before activating a trial or update an existing plan."
        )
    ...
    subcription = await subcription_plan_service.get_subscription_plans_by_tenant(
        tenant_id
    )
    await tenant_service.update_tenant(
        tenant_id=tenant_id, data=UpdateTenantDto(subscription_id=str(subcription.id))
    )
