...
@router.get(
    "/sso/{provider_name}/login", status_code=status.HTTP_307_TEMPORARY_REDIRECT
)
...
@router.get(
    "/sso/{provider_name}/callback", status_code=status.HTTP_308_PERMANENT_REDIRECT
)
async def sso_provider_callback(
    ...
    token_set = await auth_service.login_with_sso(
        provider_name=provider_name, user_info=user, tenant_id=tenant_id
    )
    response = RedirectResponse(
        url=f"{sso_auth_provider.get_redirect_uri_to_app()}/dashboard",
