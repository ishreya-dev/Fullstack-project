...
async def upload_logo(
    ...
    logger.debug(
        f"Received logo file: {file.filename}, size: {len(file_content)} bytes"
    )
    if not file_content:
    ...
    if file.content_type not in ["image/png", "image/jpeg", "image/svg+xml"]:
        raise BrandingException(
            "Unsupported file type. Please upload a PNG, JPEG, or SVG image."
        )
    ...
    if branding is None:
        ...
        branding = await branding_service.create_branding(
            data=UpdateBrandingDto(logo_url=key, logo_type=file.content_type)
        )
        return
