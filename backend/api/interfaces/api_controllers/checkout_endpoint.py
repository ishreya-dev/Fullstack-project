...
async def create_host_checkout_session(
    ...
    if current_user and current_user.tenant_id is not None:
        if checkout_req.tenant_id != current_user.tenant_id:
            logger.error(
                f"Tenant ID mismatch for the current user. And current user's tenant id is: {current_user.tenant_id} and requested tenant id is: {checkout_req.tenant_id}"
            )
            raise InvalidOperationException(
                f"Tenant ID mismatch for the current user. {checkout_req.tenant_id}"
            )

        host_main_domain = f"https://{get_host_main_domain_name()}"
        url = await billing_service.create_host_check_out_session_for_tenant(
            frontend_url=host_main_domain, checkout_req=checkout_req
        )
...
async def create_tenant_checkout_session(
    ...
    if current_user.email != checkout_req.email:
        logger.error(
            f"Email mismatch for the current user. And current user's email is: {current_user.email} and requested email is: {checkout_req.email}"
        )
        raise InvalidOperationException(
            f"Email mismatch for the current user. {checkout_req.email}"
        )
    ...
    url = await billing_service.create_tenant_check_out_session_for_tenant_users(
        frontend_url=frontend_url, user_id=current_user.id, checkout_req=checkout_req
    )
    return CheckoutResponseDto(checkout_url=url)
...
async def host_checkout_success(
    ...
    if current_user.tenant_id != tenant_id:
        logger.error(
            f"Tenant ID mismatch for the current user. And current user's tenant id is: {current_user.tenant_id} and requested tenant id is: {tenant_id}"
        )
        raise InvalidOperationException(
            f"Tenant ID mismatch for the current user. {tenant_id}"
        )

    await billing_service.handle_host_checkout_success(
...
async def host_checkout_canceled(
    tenant_id: str,
    current_user: CurrentUser,
    billing_service: BillingRecordService = Depends(get_billing_record_service),
):
    ...
    await billing_service.handle_host_checkout_canceled(
...
async def tenant_checkout_success(
    ...
    await billing_service.handle_tenant_checkout_success(
...
async def list_checkout_records(
    ...
    _bool: bool = Depends(
        check_permissions_for_current_role(
            required_permissions=[Permission.MANAGE_BILLING]
        )
    ),
    billing_service: BillingRecordService = Depends(get_billing_record_service),
):
