...
class UserService:
    ...
    async def update_user(self, user_id: str, user_data: UpdateUserDto) -> User | None:
        ...
        existing.role_id = (
            PydanticObjectId(user_data.role_id)
            if user_data.role_id
            else existing.role_id
        )
        ...
        return await self.user_repository.update(
            user_id=user_id, data=UpdateUserDto(**doc)
        )
    ...
    async def retrieve_password_reset_data_by_user_id(
        self, user_id: str
    ) -> UserPasswordReset | None:
        """Get password reset entry by token. Returns UserPasswordReset model or None if not found."""
        return await self.user_password_reset_repository.single_or_none(
    ...
    async def aggregate(self, pipeline: list[dict]) -> list[TimeSeriesDto]:
        ...
        return await self.user_repository.aggregate(
            pipeline, projection_model=TimeSeriesDto
        )
