...
from api.domain.dtos.pricing_dto import (
    CreatePricingDto,
    PricingListDto,
    UpdatePricingDto,
)
...
class PricingService:
    ...
    async def list_prices(self, scope: ScopeType) -> PricingListDto:
        ...
        return PricingListDto(
            pricings=[pricing for pricing in result.data], has_more=result.has_more
        )

    async def update_price(
        self, price_id: str, update: UpdatePricingDto, scope: ScopeType
    ) -> None:
        try:
            sc = await self.stripe_resolver.get_stripe_client(scope=scope)
            await sc.v1.prices.update_async(
                price=price_id,
                params={
