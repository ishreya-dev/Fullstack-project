...
class LocalAIService:
        ...
        for s in sessions:
            ...
            if "sessions" in serializable_dict:
                serializable_dict["sessions"] = [
                    await session.to_serializable_dict() for session in s.sessions
                ]
    ...
    async def get_histories_by_session_id(
        ...
        session = await self.chat_session_repository.single_or_none(
            session_id=PydanticObjectId(session_id), user_id=PydanticObjectId(user_id)
        )
        logger.debug(
            f"Getting histories for session_id: {session_id}, user_id: {user_id}"
        )
        if not session:
            return []
    ...
    async def save_user_query(
        ...
        if session is None:
            ...
            session = ChatSessionAI(
                session_id=(
                    PydanticObjectId(session_id) if session_id else PydanticObjectId()
                ),
