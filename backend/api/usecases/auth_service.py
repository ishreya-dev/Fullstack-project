...
class AuthService:
    ...
    async def _get_token_set(self, user: User) -> TokenSetDto:
        ...
        await self.audit_log_service.create_audit_log(
            audit_log=AuditLogDto(
                ...
                changes={"Info": "User received tokens and logged in."},
            )
        )

        # Host audit log for tenant admin login
        if user.tenant_id and Permission.FULL_ACCESS in (
            role.permissions if role.permissions else []
        ):
            logger.info(
                f"Tenant ID: {user.tenant_id} logged in. Updating an audit log in the host."
            )
            await self.audit_log_service.create_audit_log(
                audit_log=AuditLogDto(
                    entity="User",
                    ...
                    user_id=str(user.id),
                    tenant_id=None,
                    changes={"Info": "Tenant admin user logged in."},
                )
            )
        return res
        ...
        try:
            ...
            await self.audit_log_service.create_audit_log(
                audit_log=AuditLogDto(
                    ...
                    action="login",
                    user_id=user_id,
                    changes={"Info": "User attempted to log in with magic link."},
                    tenant_id=str(user.tenant_id) if user.tenant_id else None,
                )
            )
            ...
            await self.audit_log_service.create_audit_log(
                audit_log=AuditLogDto(
                    ...
                    tenant_id=str(user.tenant_id) if user.tenant_id else None,
                    changes={
                        "Info": f"User with an email: {user.email} failed to log in."
                    },
                )
            )
        ...
        try:
            ...
            await self.audit_log_service.create_audit_log(
                audit_log=AuditLogDto(
                    ...
                    user_id=str(user.id),
                    tenant_id=str(user.tenant_id) if user.tenant_id else None,
                    changes={"Info": "User attempted to log in with passkey."},
                )
            )
            return await self._get_token_set(user)
        except UserNotFoundException or Exception:
            ...
            await self.audit_log_service.create_audit_log(
                audit_log=AuditLogDto(
                    ...
                    user_id="unknown",
                    changes={
                        "Info": f"User with an email: {email} failed to log in with passkey."
                    },
                )
            )
            raise UnauthorizedException(
    ...
    async def login_with_sso(
        ...
        user_info: OpenID,
        tenant_id: PydanticObjectId | None,
    ) -> TokenSetDto:
        ...
        try:
            ...
            role: Role = await self.role_service.find_by_name(
                name=RoleType.GUEST
    ...
    async def new_user_creation(
        self, new_user: CreateUserDto, tenant_id: Optional[str] = None
    ) -> None:
        ...
        try:
            ...
            if len(enabled_sso.items) > 0:
                ...
                if user.sso_provider_id:
                    logger.debug(
                        f"This email already has logged using sso {user.sso_provider_id} hence, skip the new user creation."
                    )
                    not_found = False
        except UserNotFoundException:
                ...
                if tenant is not None:
                    await self.audit_log_service.create_audit_log(
                        audit_log=AuditLogDto(
                            ...
                            user_id="unknown",
                            changes={
                                "Info": f"Tenant with subdomain: {new_user.sub_domain} is already taken."
                            },
    ...
    async def logout(self, user_id: str, tenant_id: str | None) -> None:
        await self.audit_log_service.create_audit_log(
            audit_log=AuditLogDto(
                ...
                user_id=user_id,
                tenant_id=tenant_id,
                changes={"Info": "User logged out and refresh tokens invalidated."},
            )
        )
        logger.info(
            f"User {user_id} logged out successfully and refresh tokens invalidated."
        )
