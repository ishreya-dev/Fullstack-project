...
class TenantService:
    ...
    async def create_tenant(
        self, tenant_data: CreateTenantDto
    ...
    async def update_feature(self, tenant_id: str, feature: FeatureDto):
        ...
        await tenant.save()
        logger.debug(
            f"Feature '{feature.name}' updated to '{feature.enabled}' for tenant '{tenant_id}'"
        )
    ...
    async def update_tenant(self, tenant_id: str, data: UpdateTenantDto) -> None:
        ...
        await self.tenant_repository.update(
            tenant_id, tenant.model_dump(exclude_none=True)
        )
    ...
    async def get_features_by_tenant_id(self, tenant_id: str) -> List[FeatureDto]:
        ...
        tenant_features = [
            FeatureDto(name=feature.name, enabled=feature.enabled)
        ...
        if len(all_features) > len(tenant_features):
            logger.debug(
                f"Some features are not set for tenant {tenant_id}, adding missing features as disabled."
            )
            ...
            for all_feature in all_features:
                for tenant_feature in tenant_features:
                    ...
                    available_features.append(
                        FeatureDto(name=all_feature.name, enabled=False)
                    )
