...
class ProductService:
    def __init__(
        self, payment_repository: PaymentRepository, stripe_resolver: StripeResolver
    ):
        self.payment_repository: PaymentRepository = payment_repository
        self.stripe_resolver: StripeResolver = stripe_resolver

    async def list_products(
        self, scope: ScopeType, show_active: bool = True
    ) -> ProductListDto:
        sc = await self.stripe_resolver.get_stripe_client(scope=scope)
        result = await sc.v1.products.list_async(
            params={"limit": 100, "active": show_active}
        )
        return ProductListDto(
            products=[product for product in result.data], has_more=result.has_more
        )
    ...
    async def update_product(
        self, product_id: str, product_dto: CreateProductDto, scope: ScopeType
    ):
        try:
            sc = await self.stripe_resolver.get_stripe_client(scope=scope)
            ...
            logger.debug(
                f"Product with ID {product_id} updated successfully in {scope} scope."
            )
        except Exception as e:
            # Handle specific exceptions if needed
    ...
    async def delete_product(self, product_id: str, scope: ScopeType):
        try:
            ...
            logger.debug(
                f"Product with ID {product_id} deleted successfully in {scope} scope."
            )
        except Exception as e:
            # Handle specific exceptions if needed
