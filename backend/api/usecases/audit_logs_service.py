...
class AuditLogsService:
    def __init__(
        self, audit_log_repository: AuditLogRepository, email_service: IEmailService
    ):
    ...
    async def read_audit_logs(
        ...
        return await self.audit_log_repository.list_audit_logs(
            tenant_id=tenant_id, limit=limit, skip=skip, action=action
        )

    async def generate_audit_logs_download(
        self, action: Optional[AuditActionType] = None, tenant_id: Optional[str] = None
    ) -> List[AuditLogDto]:
        return await self.audit_log_repository.return_all_audit_logs(
    ...
    async def send_audit_log_report_via_email(
        self,
        to_email: str,
        first_name: str,
        ...
        await self.email_service.send_email(
            ...
            type="html",
            attachments=[
                UploadFile(file=attachment_data, filename=attachment_filename)
            ],
        )
