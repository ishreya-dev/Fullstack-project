...
class StorageSettingsService:
    ...
    async def configure_storage(
        self, setting: StorageSettingsDto, tenant_id: Optional[PydanticObjectId] = None
    ) -> PydanticObjectId:
    ...
    async def reset_storage(
        ...
        storage_setting: StorageSettings | None = (
            await self.storage_settings_repository.get(id=storage_id)
        )
        ...
        await self.storage_settings_repository.reset_storage(
            id=str(storage_setting.id), data=storage_setting
        )
