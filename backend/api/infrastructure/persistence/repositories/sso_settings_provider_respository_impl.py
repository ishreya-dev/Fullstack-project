...
class SSOSettingsProviderRepository(BaseRepository[SSOSettings], AuditLogRepository):
    ...
    async def update_sso_settings(
        self, id: str, data: SSOSettings
    ) -> SSOSettings | None:
        updated_sso_settings: SSOSettings | None = await super().update(
            id=id, data=SSOSettings.model_dump(data, exclude_unset=True)
        )
        return updated_sso_settings

    async def create_sso_settings(self, data: SSOSettings) -> PydanticObjectId:
        new_sso_settings: SSOSettings = await super().create(
            data=SSOSettings.model_dump(data)
        )
