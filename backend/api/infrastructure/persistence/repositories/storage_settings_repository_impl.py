...
class StorageSettingsRepository(BaseRepository[StorageSettings], AuditLogRepository):
    ...
    async def disable_all_active_storages(self) -> None:
        ...
        if total > 0:
            logger.warning(
                f"Creating multiple storage settings. Existing count: {total}"
            )
            for s in await super().search({"is_enabled": True}):
                s.is_enabled = False
                await super().update(id=str(s.id), data=s.model_dump())
                logger.info(
                    f"Disabled storage setting with ID: {s.id} for provider: {s.provider.value}"
                )
    ...
    async def configure_storage(self, setting: StorageSettings) -> PydanticObjectId:
        ...
        existing: StorageSettings | None = await self.single_or_none(
            provider=setting.provider.value
        )
        if existing is None:
            logger.info(
                f"Creating new storage setting for provider: {setting.provider.value}"
            )
            ...
            await self.add_audit_log(
                AuditLogDto(
                    action="create",
                    changes={
                        "Info": f"Created a new storage settings with {setting.provider.value} and {result.id}"
                    },
        ...
        if is_enabled_change:
            changes["is_enabled"] = {
                "from": existing.is_enabled,
                "to": setting.is_enabled,
            }
        if region_change:
        ...
        await self.add_audit_log(
            AuditLogDto(
                action="update",
                changes={
                    "Info": f"Updated storage settings with {setting.provider.value} and {result.id}",
                    **changes,
                },
                entity="StorageSettings",
    ...
    async def get_storage_by_provider(
        ...
        setting: StorageSettings | None = await super().single_or_none(
    ...
    async def reset_storage(self, id: str, data: StorageSettings) -> None:
        ...
        logger.info(
            f"Reset storage setting with ID: {id}. Update result: {result.id} document(s) modified."
        )
        await self.add_audit_log(
            AuditLogDto(
                action="update",
                changes={
                    "Info": f"Reset storage settings with {data.provider.value} and {result.id}"
                },
                entity="StorageSettings",
