    ...
    async def update_preferences(
        ...
        if existing is None:
            new_pref = UserPreference(
                user_id=PydanticObjectId(user_id), preferences=preferences
            )
            ...
            await self.add_audit_log(
                AuditLogDto(
                    ...
                    tenant_id=(
                        existing.tenant_id if existing and existing.tenant_id else None
                    ),
                )
            )
            return
        ...
        payload = UserPreference(user_id=existing.user_id, preferences=prf)
