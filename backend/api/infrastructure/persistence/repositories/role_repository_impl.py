...
class RoleRepository(BaseRepository[Role], AuditLogRepository):
    ...
    async def create(
        ...
        new_role = Role(
            name=data.name, description=data.description, tenant_id=data.tenant_id
        )
        ...
        await self.add_audit_log(
            AuditLogDto(
                action="create",
                changes={
                    "Info": f"Create new role with name {data.name} and id {result.id}"
                },
                entity="Role",
    ...
    async def update(self, role_id: str, data: UpdateRoleDto) -> Optional[Role]:
        ...
        updated_role = await super().update(
            id=role_id, data=data.model_dump(exclude_unset=True)
        )
        if updated_role:
            ...
            await self.add_audit_log(
                AuditLogDto(
                    action="update",
                    changes={
                        "new": data.model_dump(exclude_unset=True, exclude_none=True),
                        "old": existing_role_doc,
                    },
                    entity="Role",
                    tenant_id=(
                        str(updated_role.tenant_id) if updated_role.tenant_id else None
                    ),
        ...
        await self.add_audit_log(
            AuditLogDto(
                action="update",
                changes={
                    "Info": f"Attempted to update role with id {role_id}, but it was not found."
                },
                entity="Role",
                tenant_id=str(data.tenant_id) if data.tenant_id else None,
                user_id=None,  # Todo: Need to add a new property in role.. to determine who updated it.
    ...
    async def delete(self, id: str) -> bool:
        ...
        if not existing_role:
            ...
            await self.add_audit_log(
                AuditLogDto(
                    action="delete",
                    changes={
                        "Info": f"Attempted to delete role with id {id}, but it was not found."
                    },
                    entity="Role",
                    tenant_id=None,
        ...
        await self.add_audit_log(
            AuditLogDto(
                ...
                entity="Role",
                tenant_id=(
                    str(existing_role.tenant_id) if existing_role.tenant_id else None
                ),
                user_id=None,  # Todo: Need to add a new property in role.. to determine who deleted it.
            )
        )
