...
class StripeSettingsRepository(BaseRepository[StripeSettings], AuditLogRepository):
    ...
    async def store_stripe_settings(
        self, stripe_config: CreateStripeSettingDto
    ) -> None:
        """
        ...
        if settings:
            ...
            await self.add_audit_log(
                AuditLogDto(
                    action="update",
                    changes={
                        "info": f"Updated Stripe settings for tenant {settings.tenant_id}"
                    },
            ...
            logger.info("Creating new Stripe settings for tenant")
            ...
            await self.add_audit_log(
                AuditLogDto(
                    action="create",
                    changes={
                        "info": f"Created Stripe settings for tenant {settings.tenant_id}"
                    },
                    entity="StripeSettings",
                    tenant_id=str(settings.tenant_id) if settings.tenant_id else None,
        ...
        try:
            ...
            await self.add_audit_log(
                AuditLogDto(
                    action="error",
                    changes={
                        "info": f"Attempted to read Stripe settings, but an error occurred: {e}"
                    },
                    entity="StripeSettings",
                    tenant_id=None,
                    user_id=None,  # Todo: Need to add a new property in role.. to determine who added it.
