...
class NotificationBannerRepository(
    BaseRepository[NotificationBannerSetting], AuditLogRepository
):
    def __init__(self):
    ...
    async def create_banner_setting(
        self, is_enabled: bool, message: str | None, tenant_id: PydanticObjectId | None
    ) -> NotificationBannerSetting:
        new_setting = NotificationBannerSetting(
            is_enabled=is_enabled, message=message, tenant_id=tenant_id
        )
        ...
        await self.add_audit_log(
            AuditLogDto(
                action="create",
                changes={
                    "info": f"Created notification banner setting for tenant {tenant_id}"
                },
    ...
    async def update_banner_setting(
        self, id: str, is_enabled: bool, message: str | None
    ) -> NotificationBannerSetting | None:
        ...
        if not exisiting_setting:
            await self.add_audit_log(
                AuditLogDto(
                    action="error",
                    changes={
                        "info": f"Attempted to update non-existing notification banner setting with id {id}"
                    },
                    entity="NotificationBannerSetting",
                    tenant_id=None,
        ...
        updated_setting = await self.update(
            id, exisiting_setting.model_dump(exclude_none=True)
        )
        await self.add_audit_log(
            AuditLogDto(
                action="update",
                changes={
                    "new": updated_setting.model_dump(exclude_none=True),
                    "old": exisiting_setting.model_dump(exclude_none=True),
                },
                entity="NotificationBannerSetting",
                tenant_id=(
                    str(exisiting_setting.tenant_id)
                    if exisiting_setting.tenant_id
                    else None
                ),
                user_id=None,  # Todo: Need to add a new property in role.. to determine who updated it.
            )
