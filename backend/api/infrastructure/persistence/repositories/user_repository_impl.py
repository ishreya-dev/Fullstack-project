    ...
    async def update(self, user_id: str, data: UpdateUserDto) -> Optional[User]:
        try:
            ...
            updated_user = await super().update(
                id=user_id, data=data.model_dump(exclude_unset=True)
            )
            if updated_user:
                await self.add_audit_log(
                    AuditLogDto(
                        ...
                        tenant_id=(
                            str(exising_user.tenant_id)
                            if exising_user.tenant_id
                            else None
                        ),
                    )
                )
            ...
            await self.add_audit_log(
                AuditLogDto(
                    ...
                    user_id=str(user_id),
                    changes={
                        "error": f"No user found for the given user id: {user_id}"
                    },
                    tenant_id=str(data.tenant_id) if data.tenant_id else None,
                )
            )
