...
class SubscriptionPlanRepository(BaseRepository[SubscriptionPlan], AuditLogRepository):
    ...
    async def get_subscription_plan_by_user_id(
        self, user_id: str
    ) -> SubscriptionPlan | None:
        return await super().single_or_none(user_id=PydanticObjectId(user_id))

    async def get_subscription_plans_by_tenant(
        self, tenant_id: str
    ) -> SubscriptionPlan | None:
        return await super().single_or_none(tenant_id=PydanticObjectId(tenant_id))

    async def create_subscription_plan(
        self, subscription_plan: CreateSubscriptionPlanDto
    ) -> PydanticObjectId | None:
        subcription = SubscriptionPlan(
            tenant_id=(
                PydanticObjectId(subscription_plan.tenant_id)
                if subscription_plan.tenant_id
                else None
            ),
            user_id=(
                PydanticObjectId(subscription_plan.user_id)
                if subscription_plan.user_id
                else None
            ),
        ...
        await self.add_audit_log(
            AuditLogDto(
                ...
                changes={
                    "Info": "Created new subscription plan with id " + str(result.id)
                },
                tenant_id=(
                    str(subscription_plan.tenant_id)
                    if subscription_plan.tenant_id
                    else None
                ),
            )
    ...
    async def update_subscription_plan(
        self, subscription_id: str, update_data: SubscriptionPlan
    ) -> None:
        ...
        await self.add_audit_log(
            AuditLogDto(
                ...
                changes={
                    "Info": "Updated subscription plan with id " + subscription_id
                },
    ...
    async def delete_subscription_plan(self, subscription_id: str) -> None:
        ...
        if exisiting is None:
            await self.add_audit_log(
                AuditLogDto(
                    ...
                    user_id=None,
                    changes={
                        "Info": "Attempted to delete non-existing subscription plan with id "
                        + subscription_id
                    },
                    tenant_id=None,
                )
        ...
        await self.add_audit_log(
            AuditLogDto(
                ...
                changes={
                    "Info": "Deleted subscription plan with id " + subscription_id
                },
