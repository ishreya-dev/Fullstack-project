...
from api.common.utils import create_temp_file, get_logger
...
async def seed_initial_data():
    if settings.fastapi_env != "development":
        logger.info(
            "Seeding is only allowed in development environment. Skipping seeding."
        )
        return

    if settings.mongo_db_name == "myapp":
        logger.warning(
            "Using default database name 'myapp'. Please change it in production environments."
        )
    ...
    if existing_admin is None:
        ...
        await role_repo.update(
            role_id=existing_role_admin.id,
            data=UpdateRoleDto(
            ...
            fake_user = CreateUserDto(
                ...
                gender=fake.random_element(
                    elements=[
                        Gender.MALE.value,
                        Gender.FEMALE.value,
                        Gender.PREFER_NOT_TO_SAY.value,
                        Gender.OTHER.value,
                    ]
                ),
