...
async def get_current_user(
    ...
    subscription_service: SubscriptionPlanService = Depends(
        get_subscription_plan_service
    ),
) -> MeResponseDto:
    ...
    subscription = await subscription_service.get_subscription_plan_by_user_id(
        str(user.id)
    )
    me_response = MeResponseDto(
        **user_dto.model_dump(), role=role_doc, subscription=subscription
    )
    return me_response
...
async def current_user_optional(
    ...
    try:
        return await get_current_user(
            token, user_service, token_service, role_service, subscription_service
        )
