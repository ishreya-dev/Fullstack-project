...
async def _handle_download_report_shared_task_async(payload: str):
    worker_payload = WorkerPayloadDto[Dict[str, str | None]].model_validate_json(
        payload
    )
    logger.info(f"Handling task with label: {worker_payload.label}")
    ...
    if worker_payload.label == "email-sending":
        ...
        requesting_user: User = await user_service.get_user_by_id(
            user_id=worker_payload.data["requested_by"]
        )
        ...
        res = await audit_log_service.generate_audit_logs_download(
            action=worker_payload.data.get("action", None), tenant_id=ct_id
        )
