...
class FileRetrieval:
    ...
    async def generate_read_url(self, file_key: str, expires_in: int = 3600) -> str:
        ...
        """
        try:
            active_provider: StorageSettings = (
                await self.storage_repository.single_or_none(is_enabled=True)
            )
            ...
            logger.warning(
                f"Unsupported storage provider: {str(se)}, Hence using default S3 storage"
            )
            s3 = S3Storage(
