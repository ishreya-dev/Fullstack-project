...
class SSOAuthProvider:
    def __init__(
        self, sso_settings_service: SSOSettingsService, tenant_service: TenantService
    ):
    ...
    async def _provider_instance(
        self, provider_name: SSOProvider, sso_provider: SSOSettings
    ) -> fastapi_sso.SSOBase:
        domain = None
        ...
        match provider_name:
            ...
            case _:
                raise InvalidOperationException(
                    f"SSO provider '{provider_name}' is not supported."
                )
    ...
    async def _get_sso_settings(self, provider_name: SSOProvider) -> SSOSettings:
        ...
        if not sso_provider.enabled:
            ...
            raise InvalidOperationException(
                f"SSO provider '{provider_name}' is not enabled."
            )
        return sso_provider
    ...
    async def callback(
        self, provider_name: SSOProvider, req: Request
    ) -> fastapi_sso.OpenID:
        sso_provider = await self._get_sso_settings(provider_name)
        sso = await self._provider_instance(provider_name, sso_provider)
