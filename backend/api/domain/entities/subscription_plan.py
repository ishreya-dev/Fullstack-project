...
class SubscriptionPlan(ApiBaseModel):
    plan_level: Dict[str, int] = {
        ...
        "pro": 1,
        "enterprise": 2,
    }  # e.g., 0: Basic, 1: Pro, 2: Enterprise
    is_trial: bool = False
    actor: Literal["tenant", "end_user"] = (
        "tenant"  # who it is applied to, If end_userm, then user_id must be set
    )
    user_id: PydanticObjectId | None = (
        None  # The end user who has this subscription, if applicable
    )
