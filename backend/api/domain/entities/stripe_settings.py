...
class StripeSettings(ApiBaseModel):
    ...
    stripe_secret_key: Optional[str] = (
        None  # Secret key for Stripe API only tenant scope
    )

    @field_serializer("id", "tenant_id", "created_at", "updated_at")
    def serialize_as_str(self, value: PydanticObjectId | datetime | None) -> str | None:
        if value is None:
            return None
        return str(value)
...
class BillingRecord(ApiBaseModel):
    ...
    user_id: PydanticObjectId | None = (
        None  # The end user who made the payment, if applicable
    )
    payment_type: str
    currency: str
