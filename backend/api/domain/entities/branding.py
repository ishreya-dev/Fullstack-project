...
class ThemeColors(BaseModel):
    ...
    popover_foreground: str = Field(
        "oklch(0.145 0 0)", description="--popover-foreground"
    )

    primary: str = Field("oklch(0.205 0 0)", description="--primary")
    primary_foreground: str = Field(
        "oklch(0.985 0 0)", description="--primary-foreground"
    )

    secondary: str = Field("oklch(0.97 0 0)", description="--secondary")
    secondary_foreground: str = Field(
        "oklch(0.205 0 0)", description="--secondary-foreground"
    )
    ...
    sidebar_foreground: str = Field(
        "oklch(0.145 0 0)", description="--sidebar-foreground"
    )
    sidebar_primary: str = Field("oklch(0.205 0 0)", description="--sidebar-primary")
    sidebar_primary_foreground: str = Field(
        "oklch(0.985 0 0)", description="--sidebar-primary-foreground"
    )
    sidebar_accent: str = Field("oklch(0.97 0 0)", description="--sidebar-accent")
    sidebar_accent_foreground: str = Field(
        "oklch(0.205 0 0)", description="--sidebar-accent-foreground"
    )
...
class ThemeConfig(BaseModel):
    ...
    light: ThemeColors = Field(default_factory=lambda: ThemeColors())

    # Dark mode defaults explicitly set to the .dark values provided
    dark: ThemeColors = Field(default_factory=lambda: get_dark_theme_defaults())
...
class Branding(ApiBaseModel):
    ...
    def serialize_id(self, value: PydanticObjectId | datetime | None) -> str | None:
        if value is None:
            return None
