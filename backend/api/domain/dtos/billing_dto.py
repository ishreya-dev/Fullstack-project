...
from api.domain.entities.stripe_settings import ActorType, ScopeType, StatusType
...
class CreatePlanDto(BaseModel):
    currency: str = Field(
        default="eur",
        description="Must be a supported currency: https://docs.stripe.com/currencies",
    )
    interval: Literal["month", "year"]  # Add day or week later... if needed.
    product_id: str = Field(
        ...,
        description="The product whose pricing the created plan will represent. This must be the ID of an existing product.",
    )
    amount: int
