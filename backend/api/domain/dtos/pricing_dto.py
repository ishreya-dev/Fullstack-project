...
class RecurringDto(BaseModel):
    interval: Literal["day", "week", "month", "year"] = Field(
        default="month", description="Billing interval, e.g., 'month'"
    )
    interval_count: int = Field(
        default=1, description="Number of intervals between each billing cycle"
    )
...
class CreatePricingDto(BaseModel):
    ...
    unit_amount: int = Field(
        ...,
        description="A positive integer in cents (or 0 for a free price) representing how much to charge. One of unit_amount, unit_amount_decimal, or custom_unit_amount is required, unless billing_scheme=tiered.",
    )
    product: str = Field(
