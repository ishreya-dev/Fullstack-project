...
async def test_list_roles_from_host(client: AsyncClient):
    ...
    assert data.total == 4  # 4 seeded roles
...
async def test_create_role_in_host(client: AsyncClient):
    ...
    assert response.status_code == 200
    data = RoleListDto.model_validate(response.json())
    assert data.total == 5  # 4 seeded + 1 created
    # Check that the created role exists in the list
    role_ids = [r.id for r in data.roles]
    assert new_role_response.id in role_ids
...
async def test_list_of_roles_paginated_in_host(client: AsyncClient):
    ...
    assert response.status_code == 200
    data = RoleListDto.model_validate(response.json())
    assert data.total == 7  # 4 seeded + 3 created
    ...
    assert data.total == 7
    assert len(data.roles) == 1
    assert data.has_next  # 7 total, skip=2, limit=1, so there are more
    assert data.has_previous
    assert data.skip == 2
    assert data.limit == 1
